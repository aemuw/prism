...
namespace Prism.Application.Features.Articles.Queries.GetByTag
{
    public class GetByTagHandler
        : IRequestHandler<GetByTagQuery, PagedResponse<ArticleResponse>>
    {
        private readonly IArticleRepository _articleRepository;
        private readonly ISavedArticleRepository _savedArticleRepository;

        public GetByTagHandler(
        ...
        public async Task<PagedResponse<ArticleResponse>> Handle(
            GetByTagQuery query, CancellationToken cancellationToken)
        {
            var articles = await _articleRepository
                .GetByTagAsync(query.TagSlug, query.Page, query.PageSize);
